...
use crate::workspace_policy::{invalid, optional_bool, optional_str, optional_u64};
...
fn required_alias<'a>(
    ...
    first_str(input, fields).ok_or_else(|| invalid(tool_name, "missing task prompt"))
}
...
impl<C: LlmClient + Clone + 'static> Tool for AgentOpenTool<C> {
    ...
    fn execute(&self, call: &ToolCall) -> Result<ToolResult, ToolError> {
        ...
        let fork_context = optional_bool(&call.arguments, "fork_context", false, OPEN_TOOL)?;
        ...
        let assignment = prompt.to_string();
...
impl<C: LlmClient + Clone + 'static> Tool for AgentEvalTool<C> {
        ...
        crate::tool::ToolSpec::new(
            ...
            json!({
                ...
                "properties": {
                    ...
                    "name": {"type": "string", "description": "Session name from agent_open"},
                    "session_name": {"type": "string", "description": "Alias for name"},
                    "wait": {"type": "boolean", "description": "Wait until terminal (default false; blocking wait capped in sync tool path)"},
                    "timeout_ms": {"type": "integer", "description": "Wait timeout in milliseconds when wait=true"}
                },
                "additionalProperties": false
            }),
            false,
    ...
    fn execute(&self, call: &ToolCall) -> Result<ToolResult, ToolError> {
        let agent_id = optional_str(&call.arguments, "agent_id").map(str::to_string);
        let name = optional_str(&call.arguments, "name")
            .or_else(|| optional_str(&call.arguments, "session_name"))
            .map(str::to_string);
        ...
        let wait = optional_bool(&call.arguments, "wait", false, EVAL_TOOL)?;
