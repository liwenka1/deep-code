...
/// Set to any non-empty value other than `0`/`false`/`off`/`no`
/// (case-insensitive; blank counts as unset) to gate off the L2 web tools
/// (`web_search`, `fetch_url`). Env-only for now — not yet a config-file field.
