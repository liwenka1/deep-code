...
//!
//! Ported from the CodeWhale project's `codewhale-execpolicy` crate
//! (github.com/Hmbown/DeepSeek-TUI, MIT). The arity-aware allow matching is
//! reused as-is; deep-code adds a separate structured deny detector in the
//! parent module, because flags carry the danger for deny rules and arity's
//! flag-stripping would be wrong there.
//!
...
///
/// ```ignore
/// // (internal module — illustrative, not a public API path)
...
mod tests {
    ...
    fn dict_covers_at_least_30_commands() {
        // The dictionary should cover 30+ common commands.
        assert!(
            BASH_ARITY_TABLE.len() >= 30,
            "expected at least 30 entries, got {}",
            BASH_ARITY_TABLE.len()
        );
    }
}
