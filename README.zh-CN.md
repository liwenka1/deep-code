...
- **模型可申请写授权**——任务确实需要授权根之外的目录时,模型可调用 `request_write_root` 并附一句理由;审批面板显示解析后的目录与该理由(明确标注为模型的说法),由你决定——符号链接拼写在你判断之前就被解析,批准落地时还会按同一解析结果复核(提示期间被偷换的路径直接拒绝),而覆盖家目录、文件系统根,或与凭据目录重叠(`~/.ssh`、`~/.aws` 等,以及存放 API key 的 deep-code 自身 `~/.deep-code`)的申请,在问到你之前就被驳回——这与操作系统沙箱拒绝授写的是同一份清单,进程内的文件工具因此拿不到内核层面被拒的目录。批准即时生效——无需重启——且授权随会话保存(与 `--add-dir` 同款);拒绝即终局。没有任何通道能自动放行它:`yolo` 不行,`auto` 分类器不行,配置 auto-allow 不行,会话记忆也不行。
