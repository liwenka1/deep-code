...
- **Grants persist with the session**: they are stored in the session record, so `deepcode -c` restores the same boundary; `--add-dir` on a resume merges in and is saved. The record sits inside the workspace, so it is a file the model can write — the grants in it therefore carry an HMAC keyed by `~/.deep-code/session-key`, and a list that does not verify is dropped on resume with a warning rather than restored. (The key is out of reach of the model's file tools on every platform, and of a sandboxed shell on macOS and Windows; on Linux, Landlock cannot express a read denial, so a sandboxed shell can read it there — the same gap as Linux credential reads, noted above.) To add a directory mid-task, run `/add-dir DIR` in the TUI — same validation, applied and persisted on the spot — or restart with `deepcode -c --add-dir DIR`; when the model hits the boundary itself it can ask via `request_write_root`, and approving that prompt is the same grant, applied and persisted the same way.
