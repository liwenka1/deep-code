...
- **Trust that doesn't leak** — "always allow" for shell matches on command identity (program + subcommand): trusting `git push` does not wave through `git status`. Flags that change *what executes or where it writes* (`--config`, `--exec-path`, `--output`, `--target-dir`, …) break the trust match and re-prompt. Shell metacharacters (`$`, backticks, redirection, brace expansion, subshells) route to approval instead of being parsed optimistically — the gate compares written text against its rules, and anything the shell would rewrite first is excluded rather than guessed at.
