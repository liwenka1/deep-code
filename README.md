...
- **Four permission tiers** — `default` / `accept_edits` / `auto` / `yolo`, cycled with Shift+Tab. `auto` grants everything `accept_edits` does and delegates the rest to a cheap Flash classifier, behind hard floors it can never override: anything requesting network access always asks a human, a write-root request is never auto-approved, and the top risk tier never reaches the classifier — it asks unless the `accept_edits` allowance already covers it (a plain `mkdir src/x` runs unasked here exactly as it does there). `yolo` auto-approves everything the gate sees **and gives sandboxed commands ambient egress** — the per-call network prompt exists so a human sees egress requests, and with nobody in the loop it would stop no attacker (a hostile call just declares `network: true` and gets auto-approved) while stranding honest undeclared commands offline. `[sandbox] network = "never"` stays absolute even under yolo; write confinement and the deny floor are unaffected.
