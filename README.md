...
- **Four permission tiers** — `default` / `accept_edits` / `auto` / `yolo`, cycled with Shift+Tab. `auto` delegates routine approvals to a cheap Flash classifier with hard floors it can never override: top-risk commands and anything requesting network access always ask a human. `yolo` auto-approves everything the gate sees **and gives sandboxed commands ambient egress** — the per-call network prompt exists so a human sees egress requests, and with nobody in the loop it would stop no attacker (a hostile call just declares `network: true` and gets auto-approved) while stranding honest undeclared commands offline. `[sandbox] network = "never"` stays absolute even under yolo; write confinement and the deny floor are unaffected.
