...
- **Four permission tiers** — `default` / `accept_edits` / `auto` / `yolo`, cycled with Shift+Tab. `auto` grants everything `accept_edits` does and delegates the rest to a cheap Flash classifier, behind hard floors it can never override: a network request never reaches the classifier — it asks a human unless a standing consent (`auto_allow`, session "a") already covers the call; `[sandbox] network = "always"` gives sandboxed commands ambient egress with no declaration and no prompt, but a call that does declare `network: true`, or a network-native tool, still asks — a write-root request is never auto-approved, and the top risk tier never reaches the classifier — it asks unless the `accept_edits` allowance already covers it (a plain `mkdir src/x` runs unasked here exactly as it does there). `yolo` auto-approves everything the gate sees except a write-root request (which asks in every tier) **and gives sandboxed commands ambient egress** — the per-call network prompt exists so a human sees egress requests, and with nobody in the loop it would stop no attacker (a hostile call just declares `network: true` and gets auto-approved) while stranding honest undeclared commands offline. `[sandbox] network = "never"` stays absolute even under yolo — it refuses declared commands, networked dispatches and the web tools (`fetch_url`, `web_search`) alike; write confinement and the deny floor are unaffected.
- **OS sandbox (macOS / Linux)** — shell/job commands run inside macOS Seatbelt or Linux Landlock+seccomp: writes confined to the workspace, any root you granted (`--add-dir`, `/add-dir`, an approved `request_write_root`), the command's own cwd and the system temp dirs, **no network by default**. Commands that need egress (installs, `git push`, dev servers) must declare it and go through approval. `[sandbox] network = prompt|always|never` tunes this, and project-level config can only tighten it. If no sandbox backend is available, commands are refused rather than silently run bare.
  **On Linux, how completely writes are confined depends on your kernel**: Landlock gained the right governing `truncate(2)` in ABI 3 (Linux 6.2) and the one governing device `ioctl(2)` in ABI 5 (Linux 6.10), and a right the kernel cannot express is one it never checks. The two gaps are not the same gap, so they are not reported as one. Below 6.2 (Ubuntu 22.04, Debian 12, RHEL 9) every other write outside the roots is still refused — create, delete, open-for-write — so the residual is destructive (a file outside the roots can be emptied), never disclosing. Below 6.10 (Ubuntu 24.04 and most current distros) the path-write boundary is fully intact; what goes unchecked is `ioctl` on a device node, so the reach is bounded by which devices your user can open rather than by which paths were granted — and the `/dev` nodes this sandbox grants for redirection are granted *without* the ioctl right (on kernels that enforce it, this deliberately breaks pty allocation inside the sandbox — expect/pexpect-style tools — and the model is told that refusal is by design, not a path denial to fix with `/add-dir`). `deepcode doctor` names the exact gaps, the approval panel says "partial sandbox" rather than "sandboxed execution", and the model's tool description carries the sentence for the gap you actually have — it is the one issuing the write, so it is the last place a gap may be rounded, in either direction. The network guarantee is unaffected: seccomp denies the `socket`/`connect` syscalls outright, with no per-kernel right to negotiate; `io_uring` is denied under every policy (a syscall filter never sees ring submissions, so leaving it reachable would have made that denial advisory); and the syscalls that hand over another process's memory or an already-open socket — `process_vm_readv`, `pidfd_getfd` and friends — are denied alongside `ptrace`. Unprivileged user namespaces are closed in all three spellings — `unshare`/`setns` refused outright, `clone(CLONE_NEWUSER)` refused by argument filter, `clone3` answered with ENOSYS so every libc falls back to the filterable `clone(2)`; the practical cost is that browser sandboxes launched inside the agent sandbox (Puppeteer/Playwright headless Chromium) need `--no-sandbox`, the same flag they already need on distros that restrict unprivileged userns.
...
- **Model-requested write grants** — when a task genuinely needs a directory outside the granted roots, the model can call `request_write_root` with a stated reason; the approval panel shows the resolved directory and that reason (labelled as the model's claim), and you decide — a symlink spelling is resolved before you judge, the grant is re-checked against that exact resolution at approval time (a path swapped under the prompt is refused), and requests that cover the home directory or filesystem root, or that overlap a credential store (`~/.ssh`, the AWS/GCP/Azure trio, `~/.gnupg`, `~/Library/Keychains`, other agents' token stores, … and deep-code's own `~/.deep-code`, which holds the API key), bounce before you are even asked. That refusal is in-process, so it holds on every platform — and it is the floor's own work, not the sandbox's: on macOS the Seatbelt profile denies writes to the same list (by resolved path, so a symlinked `~/.aws` is covered), but on Linux this floor is the *only* one of the two, because Landlock is allow-list-only and cannot express a deny inside a granted root, and on Windows there is no filesystem confinement at all. One limit worth knowing: the floor refuses to *widen* the boundary onto those paths — it does not narrow anything, so a session started inside your home directory already has them writable. Approval widens the boundary live — no relaunch — and the grant persists with the session like an `--add-dir`, re-checked against this same floor on `-c` (the session file lives inside the workspace, so it is itself something the model can write). Denial grants nothing and tells the model not to ask again, though it can; what is guaranteed is that no channel auto-approves: not `yolo`, not the `auto` classifier, not config auto-allow, not session memory. `Enter` on that panel defaults to **deny**.
- **Trust that doesn't leak** — "always allow" for shell matches on command identity (program + subcommand): trusting `git push` does not wave through `git status`. Flags that change *what executes or where it writes* (`--config`, `--exec-path`, `--output`, `--target-dir`, …) break the trust match and re-prompt, and so does any operand that names a path outside the working directory by spelling (`git diff /dev/null ~/.ssh/id_rsa` asks; `git diff main..HEAD` does not) — the sandbox leaves reads open, so that spelling is the read fence. Shell metacharacters (`$`, backticks, redirection, brace expansion, glob patterns, subshells) route to approval instead of being parsed optimistically — the gate compares written text against its rules, and anything the shell would rewrite first is excluded rather than guessed at; which characters those are is checked against the real shell by a test, not remembered. Commands that run without a prompt — a trusted identity, a remembered session consent, an accept-edits file operation — are executed as the exact words the gate parsed, with no shell in between (`&&` and `;` are sequenced by deep-code itself); only a command a human approved as text gets shell semantics, because that text is what the human read.
...
- Shell output past the inline window is saved to a file **in full** (64 MB per-stream cap, head kept) and the result names its path — the head of a long build log (where the first error lives) stays retrievable instead of vanishing. Files live under the workspace's `.deep-code/spill/`, are skipped by checkpoints and default code search, small outputs never touch disk, and runs idle for a week are pruned at the next launch.
...
- **Grants persist with the session**: they are stored in the session record, so `deepcode -c` restores the same boundary; `--add-dir` on a resume merges in and is saved. The record sits inside the workspace, so it is a file the model can write — the grants in it therefore carry an HMAC keyed by `~/.deep-code/session-key`, and a list that does not verify is dropped on resume with a warning rather than restored — as is a list signed for a *different* workspace, so a record copied into another checkout cannot carry its grants there, and a recorded root whose spelling has come to resolve somewhere else (a symlink planted over an approved directory), which is dropped rather than silently redirected. (The key is out of reach of the model's file tools within the roots a session is given, and of a sandboxed shell on macOS. It is reachable three ways, each a consequence of something else stated on this page: a root **you** grant that contains it — a session started in `$HOME`, or `--add-dir ~` — because the floor deliberately does not police your own command line; a sandboxed shell on **Linux**, where Landlock cannot express a read denial, the same gap as Linux credential reads noted above; and any shell on **Windows**, which has no filesystem confinement at all. What the tag buys in all three is that forging a grant means reaching the key, rather than just writing the session file the model can already write.) To add a directory mid-task, run `/add-dir DIR` in the TUI — same validation, applied and persisted on the spot — or restart with `deepcode -c --add-dir DIR`; when the model hits the boundary itself it can ask via `request_write_root`, and approving that prompt is the same grant, applied and persisted the same way.
...
- **Human decision only**: there is deliberately no config key for this — a grant is the launch flag, the `/add-dir` command typed at the keyboard, or your explicit approval of the model's `request_write_root` request. A malicious repo cannot self-grant through project config, and the request path has no automatic yes: no permission mode approves it (`yolo` included), config auto-allow and session memory are ignored for it, and its prompt deliberately offers no "approve for session".
...
- **Denials don't burn tokens**: a write refused by the boundary is a failure no retry can fix, so it is handled as its own class — the first denial tells the model exactly that (teaching `request_write_root`), three in one turn stop the turn with the same guidance for you, and none of them trigger the Pro escalation that ordinary repeated tool failures do.
