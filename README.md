...
- **Model-requested write grants** — when a task genuinely needs a directory outside the granted roots, the model can call `request_write_root` with a stated reason; the approval panel shows the resolved directory and that reason (labelled as the model's claim), and you decide. Approval widens the boundary live — no relaunch — and the grant persists with the session like an `--add-dir`; denial is final. No channel auto-approves it: not `yolo`, not the `auto` classifier, not config auto-allow, not session memory.
